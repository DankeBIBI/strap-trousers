...
export async function axiosRequest(
  url: string | number,
  data: any,
  method: string | undefined = DEFAULT_METHOD
) {
...
function interceptors(Axios: {
  ...
  Axios.interceptors.request.use(
    (data: any) => {
      if (BEFORE_REQUEST) BEFORE_REQUEST!(data)
      return data
    },
    //请求失败
    (error: any) => {
      if (REQUEST_FAIL) REQUEST_FAIL!(error)
  ...
  Axios.interceptors.response.use(
    (res: any) => {
      if (REQUEST_SUCCESS) REQUEST_SUCCESS!(res.data)
      return res
    },
    //响应状态码不在2XX时
    (error: any) => {
      if (FAIL) FAIL!(error.data)
      return Promise.reject(error)
    }
  )
}
