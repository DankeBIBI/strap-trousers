...
export function encryption(data: string) {
  ...
  const go = (data_: string) => {
    ...
    list.forEach((item: any, index: any) => {
      if (item >= 0) list[index] = String(item * random1 * random2)
      if (/^[a-z]*$/g.test(item)) list[index] = formatString(item.toUpperCase())
      if (/^[A-Z]*$/g.test(item)) list[index] = formatString(item.toLowerCase())
      if (item == '%') {
        list[index] = 'DKEBIBI'
      }
    })
...
export function decrypt(data: string) {
  const go = (_data: string) => {
    let source = data.split('DKE')
    let list: Array<any> = source.reverse()
    list.forEach((item: any, index: number) => {
      if (index > 1) {
        if (item >= 0) list[index] = String(item / list[0] / list[1])
        if (/^[a-z]*$/g.test(item)) list[index] = formatString(item.toUpperCase())
        if (/^[A-Z]*$/g.test(item)) list[index] = formatString(item.toLowerCase())
        if (item == 'DKEBIBI') list[index] = ''
      }
    })
    list.splice(0, 2)
